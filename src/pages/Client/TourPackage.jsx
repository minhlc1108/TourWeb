...
const TourPackage = () => {
  ...
  useEffect(() => {
    let filtered = tourData.filter((tour) => {
      ...
      const matchesSearch = datafilter.search ? tour.title.toLowerCase().includes(datafilter.search.toLowerCase()) : true;
      const matchesBudget = datafilter.budget
      ? (datafilter.budget === "under5" && parseInt(tour.price.replace(/,/g, '')) < 5000000) ||
        (datafilter.budget === "5to10" && parseInt(tour.price.replace(/,/g, '')) >= 5000000 && parseInt(tour.price.replace(/,/g, '')) <= 10000000) ||
        (datafilter.budget === "10to20" && parseInt(tour.price.replace(/,/g, '')) > 10000000 && parseInt(tour.price.replace(/,/g, '')) <= 20000000) ||
        (datafilter.budget === "over20" && parseInt(tour.price.replace(/,/g, '')) > 20000000)
      : true;
      const matchesDepartureStart = datafilter.departureStart ? tour.departureStart.includes(datafilter.departureStart) : true;
      const matchesDepartureEnd = datafilter.departureEnd ? tour.departureEnd.includes(datafilter.departureEnd) : true;
      const matchesDepartureDate = datafilter.departureDate ? tour.date === datafilter.departureDate : true;
      const matchesCategory = datafilter.category ? tour.category === datafilter.category : true;
      const matchesTraffic = datafilter.traffic ? tour.traffic === datafilter.traffic : true;
